...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "jumpcoin.conf") + "\n";
    ...
    strUsage += HelpMessageOpt("-loadblock=<file>", _("Imports blocks from external jump000??.dat file on startup"));
    ...
    strUsage += HelpMessageOpt("-reindex", _("Rebuild chain state and block index from the jump*.dat files on disk"));
#ifndef WIN32
...
std::string LicenseInfo()
{
    ...
    const std::string URL_YouTube = "<https://www.youtube.com/user/jumperbillijumper>";
    const std::string URL_Twitter = "<https://twitter.jumperbillijumper.de>";
    const std::string URL_Telegram = "<https://telegram.jumperbillijumper.de>";
    const std::string URL_TikTok = "<https://www.tiktok.com/@jumperbillijumper>";
    const std::string URL_Facebook = "<https://facebook.jumperbillijumper.de>";
    const std::string URL_FacebookC = "<https://fbgruppe.jumperbillijumper.de>";
    const std::string URL_Instagram = "<https://instagram.jumperbillijumper.de>";
    const std::string URL_CommunityF = "<https://forum.coinerd.de>";
    const std::string URL_CommunityS = "<https://coinerd.de>";
    const std::string URL_Discord = "<https://discord.jumperbillijumper.de>";
    const std::string URL_Odysee = "<https://odysee.com/@Jumperbillijumper>";
    ...
    return FormatParagraph(strprintf("Copyright (C) %i-%i The Bitcoin Core Developers", 2009, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Blackcoin More Developers", 2014, 2023)) + "\n" +
           "\n" +
           FormatParagraph(strprintf("Copyright (C) %i-%i The Jumpcoin Developers", 2018, COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           ...
           "\n" +
           "\n" +
           FormatParagraph(strprintf("Social Media Channels")) +
           "\n" +
           "\n" +
           FormatParagraph(strprintf(_("YouTube: %s"), 
                URL_YouTube)) + "\n" + 
           FormatParagraph(strprintf(_("Twitter: %s"), 
                URL_Twitter)) + "\n" + 
           FormatParagraph(strprintf(_("Telegram: %s"), 
                URL_Telegram)) + "\n" +
           FormatParagraph(strprintf(_("TikTok: %s"), 
                URL_TikTok)) + "\n" + 
           FormatParagraph(strprintf(_("Facebook: %s"), 
                URL_Facebook)) + "\n" +  
           FormatParagraph(strprintf(_("Facebook Community Gruppe: %s"), 
                URL_FacebookC)) + "\n" +  
           FormatParagraph(strprintf(_("Instagram: %s"), 
                URL_Instagram)) + "\n" +  
           FormatParagraph(strprintf(_("Community Forum: %s"), 
                URL_CommunityF)) + "\n" +  
           FormatParagraph(strprintf(_("Community Seite: %s"), 
                URL_CommunityS)) + "\n" +  
           FormatParagraph(strprintf(_("Discord: %s"), 
                URL_Discord)) + "\n" +  
           FormatParagraph(strprintf(_("Odysee: %s"), 
                URL_Odysee)) +
           "\n" +
           "\n" +
...
void CleanupBlockRevFiles()
{
    ...
    // Glob all jump?????.dat and rev?????.dat files from the blocks directory.
    // Remove the rev files immediately and insert the jump file paths into an
    // ordered map keyed by block file index.
    LogPrintf("Removing unusable jump?????.dat and rev?????.dat files for -reindex with -prune\n");
    ...
    for (directory_iterator it(blocksdir); it != directory_iterator(); it++) {
        ...
        {
            if (it->path().filename().string().substr(0,3) == "jump")
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("jumpcoin-loadjump");
    CImportingNow imp;
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!boost::filesystem::exists(GetBlockPosFilename(pos, "jump")))
                break; // No block files left to reindex
            FILE *file = OpenBlockFile(pos, true);
            if (!file)
                break; // This error is logged in OpenBlockFile
            LogPrintf("Reindexing block file jump%05u.dat...\n", (unsigned int)nFile);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Upgrading to 0.8; hard-link the old jumpnnnn.dat files into /blocks/
    ...
    if (!boost::filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            boost::filesystem::path source = GetDataDir() / strprintf("jump%04u.dat", i);
            if (!boost::filesystem::exists(source)) break;
            boost::filesystem::path dest = blocksDir / strprintf("jump%05u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking jump%04u.dat: %s\n", i, e.what());
                break;
            }
        }
        if (linked)
