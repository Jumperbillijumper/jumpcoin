...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s jumpcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  jumpcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded jumpcoin transaction") + "\n" +
              "  jumpcoin-tx [options] -create [commands]   " + _("Create hex-encoded jumpcoin transaction") + "\n" +
              "\n";
