...
{
    QString ret = QString("jumpcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non JUMP unit input in lnReqAmount, we generate the URI with JUMP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(jumpcoinUnits::format(jumpcoinUnits::JUMP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
