                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    // Jumpcoin: in v2 transactions use GetAdjustedTime() as TxTime
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOpsCount > MAX_BLOCK_SIGOPS)
            return state.DoS(100, error("ConnectBlock(): too many sigops"),
                             REJECT_INVALID, "bad-jump-sigops");
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in jump%05u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > MAX_BLOCK_SIZE || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION) > MAX_BLOCK_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-jump-length", false, "size limits failed");
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS)
        return state.DoS(100, false, REJECT_INVALID, "bad-jump-sigops", false, "out-of-bounds SigOpCount");
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex=NULL, bool fProofOfStake=true)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevjump");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevjump");
...
void UnlinkPrunedFiles(std::set<int>& setFilesToPrune)
{
    for (set<int>::iterator it = setFilesToPrune.begin(); it != setFilesToPrune.end(); ++it) {
        ...
        boost::filesystem::remove(GetBlockPosFilename(pos, "jump"));
        boost::filesystem::remove(GetBlockPosFilename(pos, "rev"));
        LogPrintf("Prune: %s deleted jump/rev (%05u)\n", __func__, *it);
    }
}
...
void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint("prune", "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d removed %d jump/rev pairs\n",
...
FILE* OpenBlockFile(const CDiskBlockPos &pos, bool fReadOnly) {
    return OpenDiskFile(pos, "jump", fReadOnly);
}
...
bool static LoadBlockIndexDB()
{
    ...
    // Check presence of jump files
    LogPrintf("Checking all jump files are present...\n");
...
bool LoadExternalBlockFile(const CChainParams& chainparams, FILE* fileIn, CDiskBlockPos *dbp)
{
    ...
    try {
        ...
        CBufferedFile jumpdat(fileIn, 2*MAX_BLOCK_SIZE, MAX_BLOCK_SIZE+8, SER_DISK, CLIENT_VERSION);
        uint64_t nRewind = jumpdat.GetPos();
        while (!jumpdat.eof()) {
            boost::this_thread::interruption_point();

            jumpdat.SetPos(nRewind);
            nRewind++; // start one byte further next time, in case of failure
            jumpdat.SetLimit(); // remove former limit
            unsigned int nSize = 0;
            try {
                // locate a header
                unsigned char buf[MESSAGE_START_SIZE];
                jumpdat.FindByte(chainparams.MessageStart()[0]);
                nRewind = jumpdat.GetPos()+1;
                jumpdat >> FLATDATA(buf);
            ...
            try {
                // read block
                uint64_t nBlockPos = jumpdat.GetPos();
                if (dbp)
                    dbp->nPos = nBlockPos;
                jumpdat.SetLimit(nBlockPos + nSize);
                jumpdat.SetPos(nBlockPos);
                CBlock block;
                jumpdat >> block;
                nRewind = jumpdat.GetPos();
