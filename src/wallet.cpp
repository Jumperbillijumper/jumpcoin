...
// jumpcoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-jumpcoin-address
...
// JumpCoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CjumpcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CjumpcoinAddress(address).ToString());
}
...
// jumpcoin: check 'spent' consistency between wallet and txindex
// jumpcoin: fix wallet spent state according to txindex
...
// jumpcoin: disable transaction (only for coinstake)
