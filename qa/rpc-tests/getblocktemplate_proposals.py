...
def template_to_bytearray(tmpl, txlist):
    jumpver = pack('<L', tmpl['version'])
    mrklroot = genmrklroot(list(dblsha(a) for a in txlist))
    timestamp = pack('<L', tmpl['curtime'])
    nonce = b'\0\0\0\0'
    jump = jumpver + a2b_hex(tmpl['previousblockhash'])[::-1] + mrklroot + timestamp + a2b_hex(tmpl['bits'])[::-1] + nonce
    jump += varlenEncode(len(txlist))
    for tx in txlist:
        jump += tx
    return bytearray(jump)
...
class GetBlockTemplateProposalTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_template(node, tmpl, txlist, 'inconclusive-not-best-prevjump')
